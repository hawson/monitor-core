...
If InfluxDB is being used, adding the B<measurement> attribute
is I<strongly> recommended.  When present, gmond will use it as a
C<measurement> name when sending data to influxdb, set tags and values
as necessary.  By way of explanation:  InfluxDB organizes data into
C<databases>, which contain C<series> of data.  Each series contains
C<measurements> at a specific C<timestamp>.  Each measurement contains
one or more C<values>.  Gmond will convert collected metrics as best it
can to fit this data model.

If B<measurement> is not used, the metric name is re-used as the
...
metrics could look like this:
  ...
  cpu,hostname=stardust cpu_user=13.11 1460782210505334000
