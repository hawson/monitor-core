...
B<database> is the influxdb database to use.  The B<default_tags>
option indicates a list of influxdb tags to append to every metric
sent.  Tags are formatted as "key=value", and are joined with a ","
(comma) character. Embedded spaces in tags must be properlt escaped.
