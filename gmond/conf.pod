...
=head2 influxdb_send_channel

B<gmond> can be used to send data directly to an InfluxDB database
using the "Line Protocol" format supported by InfluxDB.  Messages are
sent via UDP packets.  Zero or more B<influxdb_send_channel> sections
can be included in gmond.conf.  If B<gmond> is configured as B<mute>,
the section will be ignored.

Multiple metrics will be packed into a single packet, subject to MTU
limits.  This effectively means that a single metric + metric metadata
is limited in size to < (MTU - UDP_Header), even though the Line Protocol
format allows for much larger messages.

  influxdb_send_channel {
    host     = myinfluxdb.example.com
    port     = 8089
    tags = zone=us-east,host_class=hpc  //optional tags sent with each metric
  }

The B<host> and B<port> options indicate where to send the UDP packets.
The B<tags> option indicates a list of influxdb tags to append to every
metric sent.  Tags are formatted as "key=value", and are joined with a
"," (comma) character. Values in the B<global/tags> options will also
be included.

Embedded spaces in tags must be properly escaped.  See
https://docs.influxdata.com/influxdb/v0.11/write_protocols/line/ for a
full description of the tags format.
...
If InfluxDB is being used, the B<measurement> attribute is also available,
otherwise it will be silently ignored.  When present, gmond will use it
as the C<measurement> name when sending data to influxdb.  (By way of
explanation: InfluxDB organizes data into C<databases>, which contain
C<series> of data.  Each series contains C<measurement>s at a specific
C<timestamp>s.  Each measurement further contains one or more C<values>.
Gmond will convert collected metrics as best it can to fit this data
model:  mapping channels to databases, modules and collection groups
to series and measurements within the database, and metrics to values
within a measurement.

If B<measurement> is absent, the C<MGROUP> metadata attribute from the
calling gmond module will be used if it is present.  This is the same
attribute used to set the C<GROUP> XML tag, and is also processed by
the web frontend when grouping metric graphs.

If there is no C<MGROUP> attribute, the metric name is re-used as the
InfluxDB "measurement."  This is a sub-optimal configuration, so far as
the InfluxDB schema is concerned, but it "should" work.

If a B<measurement> attribute appears in a B<collection_group> stanza,
it applies to all metrics in that group.  If it appears in a B<metric>
stanza, it will apply only to that metric.

Using the "cpu_num" example from above, when data from that group is
sent to influxdb, an InfluxDB line simliar to this will be sent:

  cpu_num,hostname=stardust cpu_num=32i 1460780807726182000

This duplicates the value "cpu_num" as the measurement.

A better configuration is to have a collection group similar to:

  collection_group {
    /* this module sets MGROUP to "cpu" */
    collect_once   = yes
    time_threshold = 1800
    metric {
      name = "cpu_num"
  ...
  collection_group {
    /* this module sets MGROUP to "mem" */
    metric { 
      name = "mem_free"
    }
    metric { 
...
Sample output lines for these three collection groups and corresponding
metrics could look like this:

  cpu,hostname=stardust cpu_num=32i 1460782210505141000
  cpu_group,hostname=stardust cpu_user=13.11 1460782210505334000
  mem,hostname=stardust mem_free=116000i 1460782210505388000
  swap,hostname=stardust swap_free=83116000i 1460782210505388000

Each collection_group has a B<measurement> attribute, with two of the
three groups having the same value.  This is okay, and in some cases
suggested configuration.  For example, network packets in/out and network
bytes in/out may be in different collection groups, but are each could
clearly be part of "net" measurement within InfluxDB.

Lastly, note that each metric stanza can have it's own B<meaurement>
attribute.  This will override the value defined in the collection group
(if present), or the metric name (in the case where the collection group
lacks a B<measurement> attribute).  This is shown in the I<swap_free>
metric in the previous example.
